...
{
    public partial class FeiBuildingDisplaceForm : Form
    {
        public FeiBuildingDisplaceForm(IMap cMap)
        {
            InitializeComponent();
            this.pMap = cMap;
        }
        ...
        #region 初始化
        private void FeiBuildingDisplaceForm_Load(object sender, EventArgs e)
        {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            List<PolygonCluster> mCluster = PolygonCluster.startAnalysis(mpolyList, double.Parse(textBox2.Text));//建筑物聚类（最后一个参数是聚类约束）
            GeoCalculation Gc = new GeoCalculation();
            List<PolygonCluster> list_a = Gc.buildingsAndRoadsConflict(mCluster, mpolineList, double.Parse(textBox1.Text));//探测类群与建筑物的冲突（最后一个参数是冲突约束）[建筑物与街道冲突]
            #endregion

            #region 建筑物群移位
            FeiDisplace Fd=new FeiDisplace();
            List<PolygonCluster> list_d=Fd.building_Road_Displacement(list_a, mpolineList, double.Parse(this.textBox1.Text), pMap.SpatialReference);

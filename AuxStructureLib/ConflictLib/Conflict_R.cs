...
namespace AuxStructureLib.ConflictLib
{
    ...
    public class Conflict_R : ConflictBase
    {
        ...
        // 给定file输出路径即可
        public void WriteConflict2(string filepath, int i, ISpatialReference pSpatialReference)
        {
            List<PolylineObject> PolylineList = new List<PolylineObject>();
            PolylineObject p = new PolylineObject();
            p.PointList = new List<TriNode>();
            ...
            p.PointList.Add(this.RightPoint);
            PolylineList.Add(p);
            List<Triangle> TriangleList = new List<Triangle>(); TriangleList.Add(this.MinTriangle);
            ...
            AuxStructureLib.Triangle.Create_WriteTriange2Shp(filepath,
                     @"ConflictTri" + i.ToString(), TriangleList,
                     pSpatialReference);
        }
    }
}

...
namespace AuxStructureLib.IO
{
    ...
    public class AEIO
    {
        ...
        public static bool Create_Write_FeatureClass(string filePath, List<PolylineObject> PolylineObjList, string fileName, ISpatialReference pSpatialReference)
        {
            #region 创建一个线的要素类文件
        ...
        /// 打开个人数据库要素
        /// </summary>
        public static void OpenFeatureClass(string filePath, string featureclassName, ESRI.ArcGIS.Controls.AxMapControl  mapControlMain)
        {
            ...
            try//打开工作空间
            {
                pWorkspaceFactory = new ESRI.ArcGIS.DataSourcesGDB.AccessWorkspaceFactoryClass();//ESRI.ArcGIS.DataSourcesFile           pFWS = pWorkspaceFactory.OpenFromFile(strFolder, 0) as IFeatureWorkspace;
                pWorkspace = pWorkspaceFactory.OpenFromFile(filePath, 0);
                pFWS = pWorkspace as IFeatureWorkspace;
                //加载矢量图层
                ...
                pFL.Name = featureclassName;
                ILayer pLayer = pFL as ILayer;
                mapControlMain.AddLayer(pLayer);
         ...
         public static void Open_WriteTriangles(string filePath, List<Triangle> TriangleList,  string featureclassName, esriSRProjCS4Type prj)
        {
         ...
         public static void Create_WritePolylineObject2Shp(string filePath, List<PolylineObject> polylineList, string fileName, esriSRProjCS4Type prj)
         {
             ...
             IFeatureWorkspace pFWS;//ESRI.ArcGIS.Geodatabase;
             IWorkspaceFactory pWorkspaceFactory = new ESRI.ArcGIS.DataSourcesFile.ShapefileWorkspaceFactory();//ESRI.ArcGIS.DataSourcesFile
             pFWS = pWorkspaceFactory.OpenFromFile(strFolder, 0) as IFeatureWorkspace;

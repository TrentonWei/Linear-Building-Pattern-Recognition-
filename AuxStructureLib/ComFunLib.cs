...
namespace AuxStructureLib
{
    public class ComFunLib
    {
        ...
        /// <returns>垂足</returns>
        public TriNode ComChuizu(TriNode s, TriNode e, TriNode p)
        {
            double x = 0, y = 0;
        ...
        /// <param name="polygon">多边形</param>
        /// <returns>是否在多边形内部</returns>
        public bool IsPointinPolygon2(TriNode p, List<TriNode> polygon)
        {
            int n = polygon.Count;
            if (n < 3)
            {
        ...
        /// <param name="isPerpendicular">最近距离是否是沿着垂线上</param>
        /// <returns>最小距离</returns>
        public double CalMinDisPoint2LineN(Node v1, Node v2, Node v3)
        {
            //点在线上的情况
            if ((v1.X == v2.X && v1.Y == v2.Y) || (v1.X == v3.X && v1.Y == v3.Y))
            {
        ...
        /// <param name="v3"></param>
        /// <returns></returns>
        public double CalMinDisPoint2LineN(TriNode v1, TriNode v2, TriNode v3)
        {
            //点在线上的情况
            if ((v1.X == v2.X && v1.Y == v2.Y) || (v1.X == v3.X && v1.Y == v3.Y))
            {
        ...
        /// <returns></returns>
        public static IFeatureLayer GetLayer(ESRI.ArcGIS.Controls.AxMapControl axMapControl1, string lyrName)
        {
            if (axMapControl1.LayerCount > 0)
            {
                for (int i = 0; i < axMapControl1.LayerCount; i++)
                {
                    if (lyrName == axMapControl1.get_Layer(i).Name)
                    {
                        return axMapControl1.get_Layer(i) as IFeatureLayer;
                    }
                }
            }
            return null;

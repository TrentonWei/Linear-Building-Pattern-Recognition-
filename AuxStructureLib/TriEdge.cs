...
namespace AuxStructureLib
{
    ...
    public class TriEdge
    {
        ...
        public int tagID = -1;//判断该条边是否是建筑物的一条边
        public int[] ptagID = { -1, -1 };        //所属目标ID
        ...
        public static int GetPointID(TriEdge edge, List<TriNode> triPoint)
        {
            ...
            {
                if (edge.RightOfLine(p))//判断是否是右边的点
                {
                    if (edge.CosValue(p) < temp)//获得余弦值最小的三角形
                    {
                        tempID = p.ID;
        ...
        public static TriNode GetPoint(TriEdge edge, List<TriNode> triPoint)
        {
            double temp = 1;
            //int tempID = 0;
        ...
        public static TriNode GetBestPoint2(TriEdge edge, List<TriNode> triPoint)
        {
            //int index = GetPointID(edge, triPoint);
            //if (index == -1)
            //    return null;
            //return TriNode.GetTriNodebyID(triPoint, index);
            return  GetPoint(edge, triPoint);
        }
        ...
        /// </summary>
        /// 修整边的LeftTriangle属性
        /// 对于三角形边集中的每一条边，若它的相反边存在，则互相标识对方为相反边；且互相标识对方的左多边形是其右多边形；否则，标识相反边为空
        /// </summary>
        /// <param name="edges"></param>
        ...
        public static void Create_WriteEdge2Shp(string filePath, string fileName, List<TriEdge> TriEdgeList, ISpatialReference pSpatialReference)
        {
